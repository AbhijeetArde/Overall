...
while cap.isOpened():
    ...
    if ret == True:
        ...
        cont, hie = cv.findContours(thresh, cv.RETR_TREE, cv.CHAIN_APPROX_SIMPLE)
                ...
                if st == True:
                    if area_cnt > 6000:
                        for i in range(defects.shape[0]):
                            ...
                            if p == True:
                                fingerP.append(ftop)
                                cv.putText(frame, "Tracking ON", (10, 90), cv.FONT_HERSHEY_SIMPLEX, 0.5, (182, 31, 102),
                                           2)
                            else:
                                cv.putText(frame, "Tracking OFF", (10, 90), cv.FONT_HERSHEY_SIMPLEX, 0.5,
                                           (182, 31, 102), 2)
                    else:
                        cv.putText(frame, "Can't detect anything", (10, 70), cv.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 150),
                                   2)
                else:
